...
router.put('/:id',async(req, res) => {
  ...
  try {
    const categoryData = await Category.update({category_name: req.body.category_name}, {where: {id: req.params.id}}); //name within the body is what we are updating by requiring the id 
    if (!categoryData) {
      res.status(404).json({ message: 'No category found with this id!' });
      return;
    }
